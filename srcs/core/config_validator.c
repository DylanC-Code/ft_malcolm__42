...
char	*config_validation_strerror(t_check_config_status err)
{
	if (err == CHECK_CONFIG_SUCCESS)
		...
		return ("Source and target IP addresses cannot be the same.");
	else if (err == CHECK_CONFIG_SAME_MAC)
		return ("Source and target MAC addresses cannot be the same.");
	else if (err == CHECK_CONFIG_LOCAL_SRC_IP)
		return ("Source IP address cannot be a local IP. (loopback, 127.*.*.*)");
	else if (err == CHECK_CONFIG_LOCAL_TGT_IP)
		return ("Target IP address cannot be a local IP. (loopback, 127.*.*.*)");
	else if (err == CHECK_CONFIG_LINK_LOCAL_SRC_IP)
		return ("Source IP address cannot be a link-local IP. (169.254.*.*)");
	else if (err == CHECK_CONFIG_LINK_LOCAL_TGT_IP)
		return ("Target IP address cannot be a link-local IP. (169.254.*.*)");
	else if (err == CHECK_CONFIG_BROADCAST_SRC_MAC)
		return ("Source MAC address cannot be a broadcast MAC. (ff:ff:ff:ff:ff:ff)");
	else if (err == CHECK_CONFIG_BROADCAST_TGT_MAC)
		return ("Target MAC address cannot be a broadcast MAC. (ff:ff:ff:ff:ff:ff)");
	else if (err == CHECK_CONFIG_MULTICAST_SRC_MAC)
		return ("Source MAC address cannot be a multicast MAC. (first byte LSB is 1)");
	else if (err == CHECK_CONFIG_MULTICAST_TGT_MAC)
		return ("Target MAC address cannot be a multicast MAC. (first byte LSB is 1)");
